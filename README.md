...
> **Beta funcional — todavía no validada para producción.** `v0.2.0-beta.3` incorpora la etapa 7.2a: consentimiento visual por sesión y captura efímera de la ventana elegida, sin guardar imágenes. Las pruebas automatizadas y la compilación no sustituyen la validación física de WGC, interfaz, teclado/lector de pantalla ni la prueba de dos horas. La identificación de hablantes de 7.2b no está implementada. El paquete de Windows no está firmado.
...
| Análisis visual temporal 7.2a | Después de asociar una ventana, exige una autorización separada para esa sesión. Permite pausar o detener solo la captura visual y descarta cada fotograma sin analizarlo ni guardarlo; todavía no identifica hablantes y su aceptación física sigue pendiente. |
...
3. Confirma tu nombre visible local, selecciona los dispositivos correctos y, si quieres registrar el proveedor, asocia manualmente una ventana superior de reunión. Autoriza el análisis visual por separado solo si deseas probar 7.2a. Descarga el modelo recomendado desde la aplicación (aproximadamente 148 MB, una vez).
...
| Captura / reproducción | NAudio 2.2.1, Windows WASAPI; Windows Graphics Capture efímero y consentido para 7.2a |
...
La evidencia histórica de `v0.1.1-mvp` registró **164/164 pruebas** y `v0.2.0-beta.2` registró **197/197** en `a8481ef`. La base funcional de `v0.2.0-beta.3`, validada en `b075958`, registra **54/54 pruebas enfocadas de captura visual**, **250/250 pruebas seriales** y compilación Release sin errores ni advertencias. Esta evidencia no demuestra WGC real, interfaz renderizada, lector de pantalla ni estabilidad física. Consulta [evidencia de validación y lista de aceptación](docs/validation.md).
...
- El análisis visual 7.2a permanece desactivado hasta una autorización separada por sesión. Los fotogramas se cierran de inmediato y no se convierten en video, capturas, registros ni evidencia de hablante.
