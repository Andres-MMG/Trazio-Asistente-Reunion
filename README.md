...
[Descargar v0.2.0-beta.12](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.12) · [Primeros pasos](docs/user-guide.md) · [Arquitectura](docs/architecture.md) · [Hoja de ruta](ROADMAP.md) · [Documentación](docs/README.md)

> **Beta funcional publicada — todavía no validada para producción.** v0.2.0-beta.12 es la descarga pública actual. Incorpora la aprobación múltiple segura 8.4b sobre la bandeja individual 8.4a. No incorpora aplicación automática del diccionario, entrenamiento, calendario ni identificación de hablantes. La validación física audible, WGC/GPU, interfaz, accesibilidad, Meet/Teams reales y las pruebas de 2/5 horas siguen pendientes. El ZIP y el Setup no están firmados.

> **Evidencia inmutable de v0.2.0-beta.12.** El tag resuelve al commit fb2aaaa207e1be4a7fcf7f0b68aa09c74e7cd0d5. El ZIP oficial mide **86,935,470 bytes** y su SHA-256 es 0bb6ebeea882d1d67afc5ceace978578197831ce2ca7fdcd4c5fb5b05464f6d2. El Setup oficial mide **60,094,013 bytes**, su SHA-256 es 54a658f9f92a750840d5621f4120e6bdc601d3a25dfd268b44add421a8a574cb y Authenticode informa NotSigned. Los seis recursos remotos coinciden en tamaño y digest. La release es una prerelease pública, no un borrador.
...
| Búsqueda local segura 8.2 | La beta 12 pública busca manualmente títulos y el texto efectivo de la transcripción original, ignorando mayúsculas y diacríticos. Descifra y compara en memoria sin FTS ni índice de texto plano; una corrección activa reemplaza el original y deshacerla lo restaura. |
| Diccionario global 8.3a | La beta 12 lista todas las entradas descifradas, filtra en memoria por términos/categoría y permite marcar cada duplicado histórico como activo o inactivo de forma independiente. Activar una entrada todavía no modifica Whisper ni las transcripciones. |
...
| Instalación manual segura | La beta 12 publica un instalador offline por usuario con payloads versionados, reparación de la misma versión, bloqueo de downgrade y rollback durante Setup. El instalador permanece sin firma y no fue ejecutado con identidad productiva; no hay descarga ni actualización automática. |

Las funciones de revisión de la etapa 8.1, publicadas desde beta 7, continúan en la beta 12 pública. Su validación audible con hardware real, teclado completo y lector de pantalla continúa pendiente.
...
1. Descarga el ZIP de Windows desde [v0.2.0-beta.12](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.12), verifica el archivo lateral .sha256 y extrae **el archivo completo**. El ZIP oficial mide **86,935,470 bytes** y su SHA-256 es 0bb6ebeea882d1d67afc5ceace978578197831ce2ca7fdcd4c5fb5b05464f6d2.
...
| Etapa 8.1 — revisión del historial | Publicada desde beta 7 y vigente en beta 12: anterior/siguiente sobre filas cargadas, línea de tiempo sensible a huecos, resaltado independiente y velocidad temporal `0,75×–2×`. Reproducción real, tono, teclado y lector de pantalla aún requieren validación física |
| Etapa 8.2 — búsqueda local | Publicada desde beta 8 y vigente en beta 12: consulta explícita, cancelable y generacional sobre títulos y texto efectivo original/corregido; máximo 100 resultados visibles, sin FTS/índice persistente y sin reproducción automática. La validación física de interfaz/accesibilidad sigue pendiente |
| Etapa 8.3a — diccionario global | Publicada desde beta 9 y vigente en beta 12: tercera pestaña, filtro local por términos/categoría y estado, máximo 120 filas visibles y activación por entrada. Activar todavía no modifica Whisper ni las transcripciones; la fusión destructiva, la aplicación del diccionario y la validación física siguen pendientes |
...
| Etapa 8.4a/8.4b — revisión pendiente | Publicada en beta 12: apertura individual sin reproducción y aprobación múltiple confirmada, atómica y anexada; sin corrección masiva ni aplicación del diccionario |
