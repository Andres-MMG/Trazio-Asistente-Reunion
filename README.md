...
[Descargar v0.2.0-beta.10](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.10) · [Primeros pasos](docs/user-guide.md) · [Arquitectura](docs/architecture.md) · [Hoja de ruta](ROADMAP.md) · [Documentación](docs/README.md)

> **Beta funcional publicada — todavía no validada para producción.** `v0.2.0-beta.10` es la descarga pública actual. Incluye la infraestructura 7.2b, el ensamblado compartido `VisualAnalysis`, el instalador manual offline por usuario, la revisión de historial 8.1, la búsqueda local 8.2, el diccionario global 8.3a y el intercambio JSON 8.3b, pero no empaqueta la CLI `VisualEvaluation`, su corpus sintético, golden ni directorios de evaluación. Los perfiles de producción para Meet y Teams permanecen `Unvalidated`; el procesamiento se abstiene y la evidencia se muestra como **No disponible** tanto en vivo como en Historial. No existe identificación de hablantes ni una capacidad empaquetada nueva. La validación física audible, WGC/GPU, interfaz, accesibilidad, importación/exportación real, Meet/Teams reales y las pruebas de 2/5 horas siguen pendientes. El ZIP y el Setup publicados no están firmados.

> **Evidencia inmutable de `v0.2.0-beta.10`.** El tag resuelve al commit `52f8b016c277a5822e9aec269fc22cc055925a2e`. El ZIP oficial `Trazio-Asistente-Reunion-v0.2.0-beta.10-win-x64.zip` mide **86,897,741 bytes** y su SHA-256 es `52d5641af82e327bbfdf510dbd732d1dee7f13a7be5294af4b993fa1dc49a42f`. El Setup oficial `Trazio-Asistente-Reunion-v0.2.0-beta.10-Setup.exe` mide **60,064,281 bytes**, su SHA-256 es `ce67f9ac2c05fa5718f99ef31339f74961af9de1a23a00c4a31ed56226977f2f` y Authenticode informa `NotSigned`. El manifiesto de publicación mide **123,577 bytes** y su SHA-256 es `f5fb80b5dce7b32dd478f2b062de3adf40d96a2db0dc254d8b6c4e1c6d6d81f4`; el manifiesto del Setup mide **488 bytes** y su SHA-256 es `f867fc695aa9de37e266621de825958f8ef9967be989cdfd2829c83d4a197a01`. Los seis recursos remotos coinciden con los artefactos verificados y sus archivos laterales. La release es una prerelease pública, no un borrador.
...
> **Revisión de historial 8.1 vigente en beta 10.** Publicada originalmente en beta 7, incorpora navegación anterior/siguiente sin reproducción automática, línea de tiempo que conserva huecos, resaltado independiente y velocidades temporales `0,75×–2×` para pista, fragmento y comparación. El resampling cambia el tono y la velocidad vuelve a `1×` al reiniciar. La prueba audible con hardware real, teclado completo y lector de pantalla continúa pendiente.

> **Diccionario global 8.3a publicado desde beta 9 y vigente en beta 10.** La tercera pestaña **Diccionario** permite listar, filtrar y activar/desactivar entradas globales cifradas. Activar una entrada solo la prepara para una aplicación futura: esta versión no aplica términos a Whisper ni reemplaza transcripciones. El paquete mantiene exactamente cinco capacidades. El instalador sigue `NotSigned`, el Setup productivo no se ha ejecutado y continúan pendientes la validación visual, por teclado, lector de pantalla y las pruebas físicas audibles, WGC/GPU, Meet/Teams y 2/5 horas.

> **Intercambio de diccionario 8.3b publicado en beta 10.** La secuencia de instalador es **11**. El formato JSON v1 exporta explícitamente sin cifrar e importa mediante vista previa, aplicación atómica y detección conservadora de duplicados/conflictos. No fusiona ni elimina entradas automáticamente, no conserva la ruta del archivo y no cambia Whisper.

> **Candidato `v0.2.0-beta.11` en `main` — todavía no publicado.** La descarga pública actual continúa siendo `v0.2.0-beta.10` y la fuente declara la secuencia de instalador **12**. La nueva bandeja **Pendientes de revisión** reúne hasta 100 segmentos de sesiones completadas o interrumpidas, permite abrir uno por vez sin reproducir audio, guardar una corrección o confirmar el original, y reabrir únicamente una aprobación original. No incorpora selección múltiple, aprobación masiva, aplicación automática del diccionario, entrenamiento ni inteligencia externa.
...
| Captura y actividad visual anónima 7.2a/7.2b | La beta 10 publicada conserva la autorización de captura WGC de 7.2a y un consentimiento distinto y de un solo uso para análisis anónimo, extracción acotada de agregados D3D11 e intervalos cifrados de cobertura/actividad. Los perfiles de producción Meet/Teams permanecen `Unvalidated`, por lo que se abstiene y muestra **No disponible**. No conserva píxeles, imágenes o video ni identifica personas. |
...
| Bandeja individual de revisión 8.4a | Incluida en el candidato beta 11 de `main`, aún no publicado: reúne pendientes de reuniones completadas o interrumpidas, muestra hasta 100 filas con truncamiento explícito y abre el original sin reproducción automática. Cualquier corrección o deshacer cuenta como intervención humana; no existen acciones masivas. |
...
| Búsqueda local segura 8.2 | La beta 10 pública busca manualmente títulos y el texto efectivo de la transcripción original, ignorando mayúsculas y diacríticos. Descifra y compara en memoria sin FTS ni índice de texto plano; una corrección activa reemplaza el original y deshacerla lo restaura. |
| Diccionario global 8.3a | La beta 10 lista todas las entradas descifradas, filtra en memoria por términos/categoría y permite marcar cada duplicado histórico como activo o inactivo de forma independiente. Activar una entrada todavía no modifica Whisper ni las transcripciones. |
...
| Instalación manual segura | La beta 10 publica un instalador offline por usuario con payloads versionados, reparación de la misma versión, bloqueo de downgrade y rollback durante Setup. El instalador permanece sin firma y no fue ejecutado con identidad productiva; no hay descarga ni actualización automática. |

Las funciones de revisión de la etapa 8.1, publicadas desde beta 7, continúan en la beta 10 pública. Su validación audible con hardware real, teclado completo y lector de pantalla continúa pendiente.
...
1. Descarga el ZIP de Windows desde [v0.2.0-beta.10](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.10), verifica el archivo lateral `.sha256` y extrae **el archivo completo**. El ZIP oficial mide **86,897,741 bytes** y su SHA-256 es `52d5641af82e327bbfdf510dbd732d1dee7f13a7be5294af4b993fa1dc49a42f`.
...
| Etapa 8.1 — revisión del historial | Publicada desde beta 7 y vigente en beta 10: anterior/siguiente sobre filas cargadas, línea de tiempo sensible a huecos, resaltado independiente y velocidad temporal `0,75×–2×`. Reproducción real, tono, teclado y lector de pantalla aún requieren validación física |
| Etapa 8.2 — búsqueda local | Publicada desde beta 8 y vigente en beta 10: consulta explícita, cancelable y generacional sobre títulos y texto efectivo original/corregido; máximo 100 resultados visibles, sin FTS/índice persistente y sin reproducción automática. La validación física de interfaz/accesibilidad sigue pendiente |
| Etapa 8.3a — diccionario global | Publicada desde beta 9 y vigente en beta 10: tercera pestaña, filtro local por términos/categoría y estado, máximo 120 filas visibles y activación por entrada. Activar todavía no modifica Whisper ni las transcripciones; la fusión destructiva, la aplicación del diccionario y la validación física siguen pendientes |
...
| Etapa 8.4a — bandeja de revisión individual | Incluida en el candidato beta 11 de `main`, todavía no publicado: pendientes globales acotados, selección única, aprobación del original y reapertura anexada; sin reproducción automática, revisión masiva ni aplicación del diccionario |
