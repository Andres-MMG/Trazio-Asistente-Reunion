...
[Descargar v0.2.0-beta.14](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.14) · [Primeros pasos](docs/user-guide.md) · [Arquitectura](docs/architecture.md) · [Hoja de ruta](ROADMAP.md) · [Documentación](docs/README.md)

> **Beta funcional publicada — todavía no validada para producción.** v0.2.0-beta.14 es la descarga pública actual. Incorpora la etapa 8.6: notas, decisiones y seguimientos manuales cifrados y vinculados a cada segmento. No incorpora entrenamiento, calendario ni identificación de hablantes. La validación física audible, WGC/GPU, interfaz, accesibilidad, Meet/Teams reales y las pruebas de 2/5 horas siguen pendientes. El ZIP y el Setup no están firmados.

> **Evidencia inmutable de v0.2.0-beta.14.** El tag resuelve al commit `5acd1def87056941e1678247deea53b0d741919b`. El ZIP oficial mide **86,955,605 bytes** y su SHA-256 es `c00c220d57ddc2bcbc52c555b18a6acc4248477536fbf9abf61b94f264de8740`. El Setup oficial mide **60,102,067 bytes**, su SHA-256 es `1ae861dd1d0b25c0adf5a7a898227bcc46d6f21a11d485d6982514bfdf459841` y Authenticode informa `NotSigned`. Los seis recursos remotos coinciden en tamaño y digest. La release es una prerelease pública, no un borrador.
...
| Búsqueda local segura 8.2 | La beta 14 pública busca manualmente títulos y el texto efectivo de la transcripción original, ignorando mayúsculas y diacríticos. Descifra y compara en memoria sin FTS ni índice de texto plano; una corrección activa reemplaza el original y deshacerla lo restaura. |
| Diccionario global 8.3a | La beta 14 lista todas las entradas descifradas, filtra en memoria por términos/categoría y permite marcar cada duplicado histórico como activo o inactivo de forma independiente. Activar una entrada solo la prepara para una retranscripción 8.5 confirmada; no reemplaza texto ni cambia el original. |
...
| Instalación manual segura | La beta 14 publica un instalador offline por usuario con payloads versionados, reparación de la misma versión, bloqueo de downgrade y rollback durante Setup. El instalador permanece sin firma y no fue ejecutado con identidad productiva; no hay descarga ni actualización automática. |

Las funciones de revisión de la etapa 8.1, publicadas desde beta 7, continúan en la beta 14 pública. Su validación audible con hardware real, teclado completo y lector de pantalla continúa pendiente.
...
1. Descarga el ZIP de Windows desde [v0.2.0-beta.14](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.14), verifica el archivo lateral `.sha256` y extrae **el archivo completo**. El ZIP oficial mide **86,955,605 bytes** y su SHA-256 es `c00c220d57ddc2bcbc52c555b18a6acc4248477536fbf9abf61b94f264de8740`.
...
| Etapa 8.1 — revisión del historial | Publicada desde beta 7 y vigente en beta 14: anterior/siguiente sobre filas cargadas, línea de tiempo sensible a huecos, resaltado independiente y velocidad temporal `0,75×–2×`. Reproducción real, tono, teclado y lector de pantalla aún requieren validación física |
| Etapa 8.2 — búsqueda local | Publicada desde beta 8 y vigente en beta 14: consulta explícita, cancelable y generacional sobre títulos y texto efectivo original/corregido; máximo 100 resultados visibles, sin FTS/índice persistente y sin reproducción automática. La validación física de interfaz/accesibilidad sigue pendiente |
| Etapa 8.3a — diccionario global | Publicada desde beta 9 y vigente en beta 14: tercera pestaña, filtro local por términos/categoría y estado, máximo 120 filas visibles y activación por entrada. La activación prepara términos para el flujo controlado 8.5; la fusión destructiva y la validación física siguen pendientes |
