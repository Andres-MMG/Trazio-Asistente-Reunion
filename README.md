...
[Descargar v0.2.0-beta.8](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.8) · [Primeros pasos](docs/user-guide.md) · [Arquitectura](docs/architecture.md) · [Hoja de ruta](ROADMAP.md) · [Documentación](docs/README.md)

> **Beta funcional publicada — todavía no validada para producción.** `v0.2.0-beta.8` es la descarga pública actual. Incluye la infraestructura 7.2b, el ensamblado compartido `VisualAnalysis`, el instalador manual offline por usuario, la revisión de historial 8.1 y la búsqueda local 8.2, pero no empaqueta la CLI `VisualEvaluation`, su corpus sintético, golden ni directorios de evaluación. Los perfiles de producción para Meet y Teams permanecen `Unvalidated`; el procesamiento se abstiene y la evidencia se muestra como **No disponible** tanto en vivo como en Historial. No existe identificación de hablantes ni una capacidad empaquetada nueva. La validación física audible, WGC/GPU, interfaz, accesibilidad, Meet/Teams reales y las pruebas de 2/5 horas siguen pendientes. El ZIP y el Setup publicados no están firmados.

> **Evidencia inmutable de `v0.2.0-beta.8`.** El tag resuelve al commit `20c94272261f5697a548c56754039029b23f1548`. El ZIP oficial `Trazio-Asistente-Reunion-v0.2.0-beta.8-win-x64.zip` mide **86,866,781 bytes** y su SHA-256 es `240a792ab8388a0511fb8b25ac466feeb104fdf6779b72938302adc9570b2e3f`. El Setup oficial `Trazio-Asistente-Reunion-v0.2.0-beta.8-Setup.exe` mide **60,034,031 bytes**, su SHA-256 es `61d87a71e2a040f10c70fa05389ca341db79e5cdcd88b2e314dcaf00c88fcaa0` y Authenticode informa `NotSigned`. El manifiesto de publicación mide **84,443 bytes** y su SHA-256 es `4844b61d0d7dcb3139acbd8489909a017a818d0b0b9a03e4a54b12db12ea0bd3`; el manifiesto del Setup mide **455 bytes** y su SHA-256 es `12afe4ddbe9eb537683fae8d1c02ae6231292470a6efd77cb38a34c95386eeaf`. Tamaños y digest de los recursos remotos coinciden con los artefactos verificados y sus archivos laterales. La release es una prerelease pública.
...
> **Revisión de historial 8.1 vigente en beta 8.** Publicada originalmente en beta 7, incorpora navegación anterior/siguiente sin reproducción automática, línea de tiempo que conserva huecos, resaltado independiente y velocidades temporales `0,75×–2×` para pista, fragmento y comparación. El resampling cambia el tono y la velocidad vuelve a `1×` al reiniciar. La prueba audible con hardware real, teclado completo y lector de pantalla continúa pendiente.

> **Candidato `v0.2.0-beta.9` en `main` — todavía no publicado.** La fuente declara la secuencia de instalador **10** e incorpora la etapa 8.3a: una pestaña **Diccionario** para listar, filtrar y activar/desactivar entradas globales cifradas. No aplica términos a Whisper, no reemplaza transcripciones y no agrega una capacidad empaquetada. La descarga pública vigente sigue siendo beta 8. El candidato aún no tiene tag, commit final, ZIP, Setup, tamaños ni SHA-256 definitivos; mantiene exactamente cinco capacidades. El instalador sigue `NotSigned`, el Setup productivo no se ha ejecutado y continúan pendientes la validación visual, por teclado, lector de pantalla y las pruebas físicas audibles, WGC/GPU, Meet/Teams y 2/5 horas.
...
| Captura y actividad visual anónima 7.2a/7.2b | La beta 8 publicada conserva la autorización de captura WGC de 7.2a y un consentimiento distinto y de un solo uso para análisis anónimo, extracción acotada de agregados D3D11 e intervalos cifrados de cobertura/actividad. Los perfiles de producción Meet/Teams permanecen `Unvalidated`, por lo que se abstiene y muestra **No disponible**. No conserva píxeles, imágenes o video ni identifica personas. |
...
| Búsqueda local segura 8.2 | La beta 8 pública busca manualmente títulos y el texto efectivo de la transcripción original, ignorando mayúsculas y diacríticos. Descifra y compara en memoria sin FTS ni índice de texto plano; una corrección activa reemplaza el original y deshacerla lo restaura. |
| Diccionario global 8.3a en el candidato beta 9 | Lista todas las entradas descifradas, filtra en memoria por términos/categoría y permite marcar cada duplicado histórico como activo o inactivo de forma independiente. Todavía no se publicó ni modifica Whisper. |
...
| Instalación manual segura | La beta 8 publica un instalador offline por usuario con payloads versionados, reparación de la misma versión, bloqueo de downgrade y rollback durante Setup. El instalador permanece sin firma y no fue ejecutado con identidad productiva; no hay descarga ni actualización automática. |

Las funciones de revisión de la etapa 8.1, publicadas desde beta 7, continúan en la beta 8 pública. Su validación audible con hardware real, teclado completo y lector de pantalla continúa pendiente.
...
1. Descarga el ZIP de Windows desde [v0.2.0-beta.8](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.8), verifica el archivo lateral `.sha256` y extrae **el archivo completo**. El ZIP oficial mide **86,866,781 bytes** y su SHA-256 es `240a792ab8388a0511fb8b25ac466feeb104fdf6779b72938302adc9570b2e3f`.
...
| Etapa 8.1 — revisión del historial | Publicada desde beta 7 y vigente en beta 8: anterior/siguiente sobre filas cargadas, línea de tiempo sensible a huecos, resaltado independiente y velocidad temporal `0,75×–2×`. Reproducción real, tono, teclado y lector de pantalla aún requieren validación física |
| Etapa 8.2 — búsqueda local | Publicada en beta 8: consulta explícita, cancelable y generacional sobre títulos y texto efectivo original/corregido; máximo 100 resultados visibles, sin FTS/índice persistente y sin reproducción automática. La validación física de interfaz/accesibilidad sigue pendiente |
| Etapa 8.3a — diccionario global | Implementada en el candidato beta 9 de `main`, todavía no publicado: tercera pestaña, filtro local por términos/categoría y estado, máximo 120 filas visibles y activación por entrada. La aplicación del diccionario, duplicados/importación/exportación y la validación física siguen pendientes |
