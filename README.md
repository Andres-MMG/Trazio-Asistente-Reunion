...
[Descargar v0.2.0-beta.15](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.15) · [Primeros pasos](docs/user-guide.md) · [Arquitectura](docs/architecture.md) · [Hoja de ruta](ROADMAP.md) · [Documentación](docs/README.md)

> **Beta funcional publicada — todavía no validada para producción.** v0.2.0-beta.15 es la descarga pública actual. Incorpora la etapa 9.1: configuración protegida de un proveedor externo opcional, todavía sin llamadas de red ni envío de transcripciones. No incorpora entrenamiento, calendario ni identificación de hablantes. La validación física audible, WGC/GPU, interfaz, accesibilidad, Meet/Teams reales y las pruebas de 2/5 horas siguen pendientes. El ZIP y el Setup no están firmados.

> **Etapa 9.2–9.6 solo en fuente.** Hay un piloto manual de evaluación privada, propuestas conservadoras, Laya local, respaldo Jev consentido, segunda escucha Qwen3-ASR y revisión de diferencias. No está en el ZIP/Setup publicado; no hay corpus real ni inferencia física verificada de esos modelos, paquete nuevo o prueba de dos/cinco horas. Consulta la [guía del piloto](docs/stage-9-transcription-pilot.md).

> **Evidencia inmutable de v0.2.0-beta.15.** El tag resuelve al commit 9a5c4af367e805fe1a8d0cd0a86722e03b1c1812. El ZIP oficial mide **86,959,816 bytes** y su SHA-256 es 0010af9e97d162835d0b109823fec00035a2013d2cd0ab8ee21fb9d75d7e4e7c. El Setup oficial mide **60,104,288 bytes**, su SHA-256 es 6fc40804fa7a3add3a2e823cb5e4c818f8fd07a39853612606e559f2aa883c8d y Authenticode informa NotSigned. Los seis recursos remotos coinciden en tamaño y digest. La release es una prerelease pública, no un borrador.
...
| Búsqueda local segura 8.2 | La beta 15 pública busca manualmente títulos y el texto efectivo de la transcripción original, ignorando mayúsculas y diacríticos. Descifra y compara en memoria sin FTS ni índice de texto plano; una corrección activa reemplaza el original y deshacerla lo restaura. |
| Diccionario global 8.3a | La beta 15 lista todas las entradas descifradas, filtra en memoria por términos/categoría y permite marcar cada duplicado histórico como activo o inactivo de forma independiente. Activar una entrada solo la prepara para una retranscripción 8.5 confirmada; no reemplaza texto ni cambia el original. |
...
| Notas y seguimiento 8.6 | Permite vincular al fragmento seleccionado una nota, decisión o seguimiento. El contenido se cifra localmente; los seguimientos se completan o reabren de forma explícita y eliminar siempre pide confirmación. No usa IA ni se exporta automáticamente. |
| Configuración externa 9.1 | La pestaña **Inteligencia** guarda URL, modelo y clave API protegidos por DPAPI. Las API remotas exigen HTTPS y HTTP solo se acepta en loopback. Guardar la configuración no realiza conexiones ni envía transcripciones; el cliente externo y el consentimiento por operación corresponden a una rebanada posterior. |
...
| Instalación manual segura | La beta 15 publica un instalador offline por usuario con payloads versionados, reparación de la misma versión, bloqueo de downgrade y rollback durante Setup. El instalador permanece sin firma y no fue ejecutado con identidad productiva; no hay descarga ni actualización automática. |

Las funciones de revisión de la etapa 8.1, publicadas desde beta 7, continúan en la beta 15 pública. Su validación audible con hardware real, teclado completo y lector de pantalla continúa pendiente.
...
1. Descarga el ZIP de Windows desde [v0.2.0-beta.15](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.15), verifica el archivo lateral .sha256 y extrae **el archivo completo**. El ZIP oficial mide **86,959,816 bytes** y su SHA-256 es 0010af9e97d162835d0b109823fec00035a2013d2cd0ab8ee21fb9d75d7e4e7c.
...
| Etapa 8.1 — revisión del historial | Publicada desde beta 7 y vigente en beta 15: anterior/siguiente sobre filas cargadas, línea de tiempo sensible a huecos, resaltado independiente y velocidad temporal 0,75×–2×. Reproducción real, tono, teclado y lector de pantalla aún requieren validación física |
| Etapa 8.2 — búsqueda local | Publicada desde beta 8 y vigente en beta 15: consulta explícita, cancelable y generacional sobre títulos y texto efectivo original/corregido; máximo 100 resultados visibles, sin FTS/índice persistente y sin reproducción automática. La validación física de interfaz/accesibilidad sigue pendiente |
| Etapa 8.3a — diccionario global | Publicada desde beta 9 y vigente en beta 15: tercera pestaña, filtro local por términos/categoría y estado, máximo 120 filas visibles y activación por entrada. La activación prepara términos para el flujo controlado 8.5; la fusión destructiva y la validación física siguen pendientes |
...
| Etapa 8.6 — notas y seguimiento | Publicada en beta 14/secuencia 15: anotaciones cifradas por segmento, tipos Nota/Decisión/Seguimiento, estado pendiente/completado, límite de 200 por reunión y eliminación explícita; validación física de interfaz/accesibilidad pendiente |
| Etapa 9.1 — configuración externa publicada | Beta 15/secuencia 16: URL/modelo/clave protegidos con DPAPI, sin cliente ni salida de transcripción en el paquete publicado |
| Etapa 9.2–9.6 — piloto manual en fuente | Arnés offline sin corpus real, generador local/remoto con consentimiento del JSON exacto, evaluación Laya o Jev acotado, Qwen3-ASR del mismo intervalo y revisión humana. No publicado ni probado físicamente con modelos; sin aplicación automática ni resúmenes/traducción/temas |
...
- En **beta 15 publicada**, el audio y la transcripción permanecen locales; no hay inferencia en la nube implementada en ese paquete. El **piloto solo en fuente** sí puede enviar texto a un proveedor remoto tras mostrar el JSON exacto y recibir consentimiento por operación; el audio no se envía por ese cliente.
