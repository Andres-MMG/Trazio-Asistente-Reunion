...
> **Estado:** la versión publicada `v0.2.0-beta.5` incluye 7.2a, la infraestructura fuente de 7.2b y el ensamblado compartido `VisualAnalysis`. La verificación independiente aprobó metadatos **4/4**, evaluación visual **74/74**, captura visual **206/206**, Release serial/paralelo **446/446**, compilación limpia, CLI `VE000`, `publish` y smoke IPC integrado/explícito. El layout final **495/495** excluye `VisualEvaluation`, el [corpus sintético agregado y su golden](../evaluation/stage-7b/README.md), `tools` y `evaluation`; mantiene cinco capacidades y cero hallazgos prohibidos, rutas locales o CodeView. La release es prerelease, no borrador, y su tag resuelve a `5f16631747dd7f7a7f68d49ba0ca9cbd659f2733`. La evaluación sintética no es calibración física: Meet/Teams permanecen `Unvalidated`, se abstienen y muestran **No disponible**; no existe identificación de hablantes. Siguen pendientes WGC/GPU/interfaz/accesibilidad/Meet/Teams físicos, 2/5 horas y firma.
...
- La evidencia visual anónima no se exporta en beta 5. TXT, Markdown y Obsidian mantienen la transcripción y `SpeakerName` existentes sin agregar actividad visual.
...
- [x] El código 7.2a/7.2b no crea videos, screenshots, dumps propios, logs sensibles ni registros de píxeles; el script y la inspección del ZIP beta 5 publicado rechazaron esas clases. Falta inspeccionar la ejecución física.
...
| Problema de privacidad | OFF por defecto, consentimiento consumible, cero retención de frames y ninguna exportación de evidencia en beta 5 |
