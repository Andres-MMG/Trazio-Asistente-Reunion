...
> **Estado:** la versión publicada `v0.2.0-beta.4` incluye 7.2a y la infraestructura fuente de 7.2b. `0.2.0-beta.5` continúa como candidato local: la verificación independiente aprobó metadatos **4/4**, evaluación visual **74/74**, captura visual **206/206**, Release serial/paralelo **446/446**, compilación limpia, CLI `VE000`, `publish` local y smoke IPC integrado/explícito. El layout preliminar **495/495** incluyó `VisualAnalysis` y excluyó `VisualEvaluation`, el [corpus sintético agregado y su golden](../evaluation/stage-7b/README.md), `tools` y `evaluation`; mantuvo cinco capacidades y cero hallazgos prohibidos, rutas locales o CodeView. No es calibración física ni publicación. Meet/Teams permanecen `Unvalidated`, se abstienen y muestran **No disponible**; no existe identificación de hablantes. Aún deben reconstruirse los activos tras el commit de preparación y registrar tamaño/SHA-256 finales, SHA, tag y release beta 5; también siguen pendientes WGC/GPU/interfaz/accesibilidad/Meet/Teams físicos, 2/5 horas y firma.
...
La evidencia histórica de 7.2a registra **54/54 pruebas enfocadas** (`Area=VisualCapture`) y la base funcional beta 3 `b075958` registra **250/250 pruebas seriales** con compilación Release sin errores ni advertencias. La beta 4 publicada completó **4/4 `VersionMetadataTests`**, **132/132 pruebas `Area=VisualCapture`**, **371/371 pruebas Release seriales**, **371/371 pruebas Release en paralelo predeterminado** y una compilación con **0 advertencias y 0 errores**. El contrato de publicación y la prueba básica por canal con nombre aprobaron; el layout coincidió en **494/494 archivos byte a byte**, con **0** hallazgos prohibidos, **0** rutas fuente locales y **0** referencias CodeView. El tag corresponde a `f871f20c3bf9e77b0cf9ad51134febb83c673de7`, y el tamaño/digest del ZIP remoto coinciden con el paquete local y el archivo lateral publicado. El código fuente posterior incorpora una regresión sintética versionada que cubre histéresis, límites temporales, discontinuidades, disponibilidad, exclusión de micrófono, falso positivo deliberado, latencia y presupuestos; `verify` compara el informe canónico byte a byte. La verificación final independiente de esta fuente aprobó **74/74 pruebas `VisualEvaluation`**, **206/206 pruebas `Area=VisualCapture`**, **446/446 pruebas Release seriales**, **446/446 pruebas Release en paralelo predeterminado**, el comando CLI `verify` y una compilación con **0 advertencias y 0 errores**. El golden canónico mide **23.193 bytes** y su SHA-256 es `6BBA6F4BA88482FE6616E4145C0840EBAD2BE0A34C9F20832B44500C13BE1738`. Esta evidencia corresponde a comprobaciones de código fuente y datos sintéticos posteriores a beta 4: no ejecuta una captura WGC/GPU real ni valida el escritorio renderizado, accesibilidad, Meet/Teams o duración, y todavía no pertenece a una versión publicada.

Para el candidato beta 5, la misma verificación independiente agregó `VersionMetadataTests` **4/4**, `publish` local y smoke IPC integrado/explícito aprobados, además de un layout preliminar **495/495** archivos byte a byte. `VisualAnalysis.dll` estuvo presente y versionada; evaluador, corpus/golden, `tools` y `evaluation` estuvieron ausentes; se conservaron cinco capacidades y hubo **0** hallazgos prohibidos, rutas fuente locales o referencias CodeView. Esta evidencia aún no fija el activo final: el ZIP debe reconstruirse después del commit de preparación y siguen pendientes su tamaño/SHA-256 finales, el SHA de preparación, tag y release beta 5.
