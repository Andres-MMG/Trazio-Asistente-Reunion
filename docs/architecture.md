...
Esto describe la implementación `0.1.1-mvp`, no una arquitectura objetivo. Las capacidades futuras están en la [hoja de ruta](../ROADMAP.md).
...
Fuentes oficiales de versiones: [App](../src/Trazio.AsistenteReunion.App/Trazio.AsistenteReunion.App.csproj), [Core](../src/Trazio.AsistenteReunion.Core/Trazio.AsistenteReunion.Core.csproj), [Worker](../src/Trazio.AsistenteReunion.Worker/Trazio.AsistenteReunion.Worker.csproj), [Pruebas](../tests/Trazio.AsistenteReunion.Tests/Trazio.AsistenteReunion.Tests.csproj), [metadatos de versión](../Directory.Build.props). La versión de parche del SDK instalado depende del entorno; ningún `global.json` la fija.
