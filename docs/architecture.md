...
La versión pública actual `0.2.0-beta.10` separa la lógica determinista en el ensamblado puro y sin paquetes `VisualAnalysis`, incluido y versionado en el layout final, deja la CLI offline `VisualEvaluation`, el corpus sintético agregado y el golden canónico fuera del paquete e incorpora la revisión de historial 8.1, la búsqueda local 8.2, el diccionario global 8.3a y el intercambio JSON 8.3b. La verificación aprobó Release serial/paralelo **599/599**, la suite enfocada de 8.3b **48/48**, los contratos finales **22/22**, el harness desechable **14/14** y la compilación con **0 advertencias y 0 errores**. El layout coincidió **495/495** con `ProductVersion` `0.2.0-beta.10+52f8b016c277a5822e9aec269fc22cc055925a2e` y exactamente cinco capacidades. Los perfiles de producción para Google Meet y Microsoft Teams permanecen `Unvalidated`, por lo que el detector se abstiene y la interfaz presenta **No disponible**. No existe identificación de hablantes ni una nueva capacidad empaquetada; activar una entrada del diccionario todavía no modifica Whisper ni las transcripciones. Siguen pendientes validación física audible, importación/exportación real, WGC/GPU/interfaz/accesibilidad/Meet/Teams/2 h/5 h y firma.

El tag público de beta 10 resuelve a `52f8b016c277a5822e9aec269fc22cc055925a2e`. El ZIP de **86,897,741 bytes** y SHA-256 `52d5641af82e327bbfdf510dbd732d1dee7f13a7be5294af4b993fa1dc49a42f`, y el Setup de **60,064,281 bytes** y SHA-256 `ce67f9ac2c05fa5718f99ef31339f74961af9de1a23a00c4a31ed56226977f2f`, coinciden con los recursos remotos y sus sidecars. El Setup informa `NotSigned`; el manifiesto de publicación mide **123,577 bytes** y su SHA-256 es `f5fb80b5dce7b32dd478f2b062de3adf40d96a2db0dc254d8b6c4e1c6d6d81f4`, y el manifiesto del Setup mide **488 bytes** con SHA-256 `f867fc695aa9de37e266621de825958f8ef9967be989cdfd2829c83d4a197a01`. Los seis recursos remotos fueron verificados.

La beta 10 usa la secuencia de instalador **11** y conserva el empaquetador ZIP canónico, que relee el manifiesto del payload y normaliza rutas, orden y fecha. La identidad byte a byte del ZIP se exige solo con la misma compilación exacta de PowerShell/.NET. El manifiesto conserva exactamente cinco capacidades. No hay firma Authenticode ni ejecución productiva del Setup, y toda validación física audible, WGC/GPU, interfaz/accesibilidad, Meet/Teams y 2/5 horas permanece pendiente.

La etapa 8.3a publicada incorpora el espacio global de diccionario sobre el esquema cifrado existente. Lista y filtra términos descifrados en memoria, y permite cambiar únicamente `is_active`; no recifra los blobs ni altera su procedencia. Una entrada activa queda preparada para una aplicación futura, pero beta 10 todavía no la aplica a Whisper ni reemplaza transcripciones.
...
### Límite de revisión pendiente 8.4a
...
| Decisión de revisión individual | Evento anexado `ApproveOriginal`/`Reopen`, revisión monotónica por segmento, revisor local cifrado y fecha; no copia título ni texto |
