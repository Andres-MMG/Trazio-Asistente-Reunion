...
La versión pública actual 0.2.0-beta.13 incorpora la aplicación controlada 8.5 del diccionario sobre retranscripciones versionadas y conserva la aprobación múltiple segura 8.4b. Conserva exactamente cinco capacidades; los perfiles Meet/Teams permanecen Unvalidated, no existe identificación de hablantes y siguen pendientes las validaciones físicas y de 2/5 horas.

El tag público de beta 13 resuelve a `512d6cccc631f22505d52f62cfbb0443db42394a`. El ZIP de **86,940,830 bytes** y SHA-256 `a5d8d83d048167e2d9ccfa1ad21ed676278af49bd00253c206bf7bdda8ec5220`, y el Setup de **60,084,574 bytes** y SHA-256 `405c4292294933628736b060009bd4a321139fec31adc6622540530f8c2d4eec`, coinciden con los recursos remotos y sus sidecars. El Setup informa `NotSigned`; el manifiesto de publicación mide **84,445 bytes** y su SHA-256 es `29f22b3d9622ab0dba9310dc001801b368ff0e5a4be8b4e5b06229fa28b71a9d`, y el manifiesto del Setup mide **458 bytes** con SHA-256 `4a9bedff74f77b2ceeb7128cdc9763fe8ccb3c81f14b0f7fda2f2cf3bfc9e97b`. Los seis recursos remotos fueron verificados.

La beta 13 usa la secuencia de instalador **14** y conserva el empaquetador ZIP canónico, que relee el manifiesto del payload y normaliza rutas, orden y fecha. La identidad byte a byte del ZIP se exige solo con la misma compilación exacta de PowerShell/.NET. El manifiesto conserva exactamente cinco capacidades. No hay firma Authenticode ni ejecución productiva del Setup, y toda validación física audible, WGC/GPU, interfaz/accesibilidad, Meet/Teams y 2/5 horas permanece pendiente.

La etapa 8.3a incorpora el espacio global de diccionario sobre el esquema cifrado existente. Lista y filtra términos descifrados en memoria, y permite cambiar únicamente `is_active`; no recifra los blobs ni altera su procedencia. La etapa 8.5 usa las entradas activas solo después de una confirmación explícita para construir un prompt local; no reemplaza texto ni modifica la revisión original.
...
Las etapas 8.4a/8.4b publicadas en beta 12 añaden una bandeja local de pendientes y aprobación múltiple explícita. La etapa 8.5 publicada en beta 13 añade orientación local y confirmada de Whisper durante una retranscripción; no amplía el manifiesto de capacidades ni introduce red, LLM, calendario, diarización o identificación de hablantes.
...
|---|---|---|
| `Trazio.AsistenteReunion.App` | Interfaz WPF, audio de Windows, ciclo de vida de sesión, búsqueda/presentación de historial, bandeja individual de revisión, captura visual efímera consentida, sondeo agregado 7.2b y presentación fail-closed en vivo/Historial | [App.xaml.cs](../src/Trazio.AsistenteReunion.App/App.xaml.cs), [MainWindow.xaml.cs](../src/Trazio.AsistenteReunion.App/MainWindow.xaml.cs), [HistorySearchPresentation.cs](../src/Trazio.AsistenteReunion.App/HistorySearchPresentation.cs), [BatchReviewPresentation.cs](../src/Trazio.AsistenteReunion.App/BatchReviewPresentation.cs), [RecordingCoordinator.cs](../src/Trazio.AsistenteReunion.App/RecordingCoordinator.cs), [MeetingWindowSelection.cs](../src/Trazio.AsistenteReunion.App/MeetingWindowSelection.cs), [VisualCaptureSessionController.cs](../src/Trazio.AsistenteReunion.App/VisualCaptureSessionController.cs), [AnonymousVisualAnalysis.cs](../src/Trazio.AsistenteReunion.App/AnonymousVisualAnalysis.cs), [AnonymousVisualEvidencePresentation.cs](../src/Trazio.AsistenteReunion.App/AnonymousVisualEvidencePresentation.cs) |
| `Trazio.AsistenteReunion.Core` | Registros de dominio, cifrado, persistencia, búsqueda local y revisión pendiente acotadas, traslado de almacenamiento, catálogo de modelos, recuperación, IPC y marcadores de proceso | [Domain.cs](../src/Trazio.AsistenteReunion.Core/Domain.cs), [SqliteSessionStore.cs](../src/Trazio.AsistenteReunion.Core/SqliteSessionStore.cs), [HistorySearchDomain.cs](../src/Trazio.AsistenteReunion.Core/HistorySearchDomain.cs), [SqliteHistorySearchStore.cs](../src/Trazio.AsistenteReunion.Core/SqliteHistorySearchStore.cs), [BatchReviewDomain.cs](../src/Trazio.AsistenteReunion.Core/BatchReviewDomain.cs), [SqliteBatchReviewStore.cs](../src/Trazio.AsistenteReunion.Core/SqliteBatchReviewStore.cs), [StorageLocation.cs](../src/Trazio.AsistenteReunion.Core/StorageLocation.cs), [ApplicationRunningMarker.cs](../src/Trazio.AsistenteReunion.Core/ApplicationRunningMarker.cs) |
...
### Límite de revisión pendiente 8.4a/8.4b

[SqliteBatchReviewStore](../src/Trazio.AsistenteReunion.Core/SqliteBatchReviewStore.cs) recorre los segmentos de sesiones completadas o interrumpidas, excluye cualquier segmento con historial `SetText`/`Undo` y toma la última decisión anexada. Conserva como máximo 100 filas visibles, pero completa la lectura y el descifrado para informar el total/truncamiento y fallar cerrada ante corrupción. No crea FTS, índice de texto, caché persistente ni registro con contenido o identificadores.

[BatchReviewPresentation](../src/Trazio.AsistenteReunion.App/BatchReviewPresentation.cs) presenta reunión, fecha, fuente, tiempo, fragmento y estado con selección única y nombres accesibles. La consulta cifrada se despacha fuera del Dispatcher WPF con un token enlazado; las cargas son cancelables y generacionales, y una generación obsoleta nunca publica. Abrir un pendiente reutiliza la navegación estable por sesión/segmento, selecciona la revisión original y nunca reproduce audio automáticamente. El borrador queda anclado a su sesión, segmento y fuente: cambiar segmento, sesión, pista o revisión queda bloqueado hasta guardar o descartar; cambiar de pestaña no lo elimina y cerrar exige confirmar el descarte.

### Límite de diccionario en retranscripción 8.5

[GlossaryPromptPlanner](../src/Trazio.AsistenteReunion.Core/GlossaryPromptPlanner.cs) toma una instantánea local de las entradas activas, normaliza para detectar conflictos y excluye por completo cada forma equivocada que apunte a más de un término preferido. Colapsa términos preferidos equivalentes, ordena de forma determinista y conserva como máximo 64 términos y 1.024 caracteres. El prompt contiene solo los términos preferidos; nunca incluye las formas equivocadas.

Antes de iniciar, [MainWindow](../src/Trazio.AsistenteReunion.App/MainWindow.xaml.cs) presenta hasta ocho correspondencias y ofrece **Sí**, **No** o **Cancelar**, con Cancelar como valor seguro. Sí usa el prompt; No crea la revisión sin diccionario; Cancelar no crea una ejecución. No existe sustitución posterior de palabras ni escritura de correcciones o entradas de glosario.

[HistoryRetranscriptionService](../src/Trazio.AsistenteReunion.App/HistoryRetranscriptionService.cs) congela el plan confirmado, crea una revisión separada y envía el prompt opcional solo en la solicitud `start` del canal local. El [Worker](../src/Trazio.AsistenteReunion.Worker/Program.cs) rechaza prompts sobre el límite o con caracteres de control y aplica `WithPrompt` al construir el procesador de Whisper. [SqliteModelRevisionStore](../src/Trazio.AsistenteReunion.Core/SqliteModelRevisionStore.cs) cifra una huella versionada SHA-256 del prompt, no su contenido en texto plano. La etiqueta de revisión declara si usó diccionario confirmado. El prompt orienta la inferencia, pero no garantiza una corrección.
### Límite de notas y seguimiento 8.6

[SqliteSegmentAnnotationStore](../src/Trazio.AsistenteReunion.Core/SqliteSegmentAnnotationStore.cs) conserva anotaciones manuales enlazadas a la sesión y al segmento original. El contenido se autentica y cifra con AES-256-GCM; el esquema limita los tipos a Nota, Decisión y Seguimiento, y solo Seguimiento admite los estados Pendiente/Completado. La aplicación limita cada reunión a 200 anotaciones y muestra hasta 50 por segmento, más recientes primero.
...
No hay dependencia HTTP, cliente de proveedor ni llamada externa en la rebanada 9.1 publicada. El piloto posterior implementa una frontera separada: construir un payload acotado, mostrar el texto exacto que saldrá, obtener consentimiento para esa operación y registrar proveedor/modelo sin registrar el secreto. La grabación, la transcripción local y la revisión deben seguir funcionando aunque el proveedor no esté configurado o falle.

### Piloto manual de transcripción mejorada 9.2–9.6 (solo fuente)

La implementación conserva tres contratos separados: **transcriptor acústico**, **generador textual de propuestas** y **evaluador de candidatos**. El original de Whisper y las correcciones humanas permanecen; [SqliteTranscriptRefinementStore](../src/Trazio.AsistenteReunion.Core/SqliteTranscriptRefinementStore.cs) agrega lotes/propuestas cifrados ligados a la versión original del segmento. Aceptar una propuesta valida la revisión esperada y registra la corrección y la decisión en una transacción; rechazar no altera el texto. Los juicios se almacenan por separado en [SqliteTranscriptRefinementEvaluationStore](../src/Trazio.AsistenteReunion.Core/SqliteTranscriptRefinementEvaluationStore.cs). Esta migración es aditiva, no reinterpreta reuniones antiguas.

[ConservativeTranscriptRefinementGenerator](../src/Trazio.AsistenteReunion.App/ConservativeTranscriptRefinementGenerator.cs) usa un servicio compatible con chat completions configurado en 9.1, con contexto cercano y diccionario aprobados y acotados. Produce hasta dos propuestas conservadoras o ninguna. Para destino remoto muestra el JSON exacto y exige autorización por operación; guardar la configuración 9.1 todavía no envía nada. La propuesta no escucha audio. [MainWindow.Refinement](../src/Trazio.AsistenteReunion.App/MainWindow.Refinement.cs) mantiene la selección y la revisión manual dentro de Historial, sin reemplazo automático.

[RefinementLayaEvaluator](../src/Trazio.AsistenteReunion.App/RefinementLayaEvaluator.cs) pregunta a un [sidecar Node local](../tools/Trazio.Laya.Sidecar/server.mjs) por significado, hechos no respaldados y preferencia. En la comparación conjunta v2, una revisión Qwen válida y producida explícitamente mediante llama.cpp puede acompañar al original y las propuestas; conserva marcas y procedencia de hipótesis acústica. Laya no oye el audio: probabilidades y recomendación no prueban fidelidad. [RefinementJevEvaluator](../src/Trazio.AsistenteReunion.App/RefinementJevEvaluator.cs) es respaldo remoto manual, con clave DPAPI distinta y vista previa/consentimiento del JSON exacto, solo si una comprobación nueva muestra Laya no instalado, no disponible o sin capacidad. No sustituye a Laya por incertidumbre y no recibe la comparación conjunta Qwen en este piloto.

[LlamaCppAsrTransport](../src/Trazio.AsistenteReunion.App/LlamaCppAsrTransport.cs) opera sobre un intervalo original elegido, por loopback, con proceso, token y revisión separada; [HistoryRetranscriptionService](../src/Trazio.AsistenteReunion.App/HistoryRetranscriptionService.cs) rechaza intervalos no continuos. Las grabaciones nuevas agregan identidad de ejecución, época y desplazamiento de muestras; el archivo usa una tolerancia de reloj provisional de 40 ms para distinguir jitter de un hueco. Los audios antiguos sin esos metadatos siguen con rechazo estricto en límites ambiguos. Esto no es una calibración física de pérdida WASAPI.

El [arnés offline](../evaluation/stage-9/README.md) compara hipótesis suministradas sobre los mismos WAV privados con consentimiento: WER/CER, latencia/memoria suministradas y etiquetas humanas de cambio de significado. No ejecuta modelos ni extrae reuniones. La [guía del piloto](stage-9-transcription-pilot.md) separa esta capacidad en fuente de beta 15 publicada y registra las pruebas físicas pendientes.
...
| Decisión de revisión individual o múltiple | Evento anexado ApproveOriginal/Reopen, revisión monotónica por segmento, revisor local cifrado y fecha; el lote solo agrega ApproveOriginal y no copia título ni texto |
| Entrada de glosario | Forma incorrecta → término preferido, categoría, estado activo y procedencia de corrección de transcripción o de archivo importado mediante un lote opaco |
...
El esquema se inicializa y actualiza en [SqliteSessionStore](../src/Trazio.AsistenteReunion.Core/SqliteSessionStore.cs), [SqliteReviewStore](../src/Trazio.AsistenteReunion.Core/SqliteReviewStore.cs), [SqliteBatchReviewStore](../src/Trazio.AsistenteReunion.Core/SqliteBatchReviewStore.cs) y [SqliteModelRevisionStore](../src/Trazio.AsistenteReunion.Core/SqliteModelRevisionStore.cs). SQLite utiliza WAL y claves foráneas. Los metadatos permanecen visibles; el contenido seleccionado se cifra antes de insertarse. Las decisiones 8.4a revalidan cada escritura. En 8.4b, una transacción inmediata valida todos los segmentos antes de insertar; cualquier conflicto revierte el lote completo y no se reintenta silenciosamente. Consulta [seguridad](security.md).
...
5. [GlossaryCandidateExtractor](../src/Trazio.AsistenteReunion.Core/GlossaryCandidateExtractor.cs) propone los términos cambiados por una edición humana, no toda la oración sin editar. [GlossaryWorkspacePresenter](../src/Trazio.AsistenteReunion.App/GlossaryWorkspacePresentation.cs) ordena y filtra en memoria la lista global con un límite visual de 120. [SqliteReviewStore](../src/Trazio.AsistenteReunion.Core/SqliteReviewStore.cs) actualiza únicamente `is_active` por ID; términos, categoría, procedencia, fecha y blobs cifrados permanecen intactos. Una operación con cancelación propia serializa carga/toggle, se drena al salir/cerrar y solo publica si la pestaña sigue vigente. Las entradas activas todavía no modifican la inferencia.
...
La beta 15 no incluye clientes LLM ni Jev; el piloto 9.2–9.6 solo en fuente agrega clientes opcionales, no dependencias del paquete público. Los adaptadores de proveedores de reuniones y los calendarios siguen planificados. Los futuros flujos de datos requieren consentimiento explícito y trabajo de seguridad independiente; consulta las [etapas 7–12](../ROADMAP.md#etapa-7--fuente-de-reunión-y-atribución-de-hablantes).
