...
La fuente de trabajo declara el candidato local `0.2.0-beta.6`, secuencia de instalador **7**, pero todavía no tiene tag ni release. La descarga pública continúa siendo beta 5; la evidencia y los artefactos locales del candidato no cambian los datos históricos de esa publicación.
...
La implementación de etapa 5.1 está incluida en el candidato local beta 6/secuencia 7. El manifiesto del payload es determinista y los archivos laterales validan la integridad del artefacto observado; no autentican al editor, y el Setup se construyó como `NotSigned`. El rollback solo cubre el fallo/cancelación antes de completar Setup. No incluye el primer arranque posterior ni garantiza compatibilidad de datos hacia atrás. La definición del instalador no lee, copia, migra ni borra la raíz de datos; las pruebas automatizadas verifican el contrato y datos sintéticos, no la preservación física de cualquier contenido arbitrario.
