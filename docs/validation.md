...
La versión publicada actual es [`v0.2.0-beta.5`](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.5), un prerelease no borrador. El tag resuelve al commit de preparación `5f16631747dd7f7a7f68d49ba0ca9cbd659f2733`. El ZIP oficial mide **86,829,207 bytes** y su SHA-256 es `0031cab096013b7bb436221a75d874719cc1d47ed03ac4047000b4e3094ddbdf`; tamaño y digest del recurso remoto coinciden con el paquete verificado y el archivo lateral publicado. Se conserva la evidencia histórica de beta 3 y beta 4. Ninguna de estas comprobaciones sustituye evidencia física.

La fuente actual declara un candidato local `0.2.0-beta.6`, secuencia **7**, aún sin tag ni release; la descarga pública sigue siendo beta 5. Los resultados y hashes locales siguientes no se atribuyen a beta 5 ni prueban una publicación o instalación física.
...
| Candidato fuente local beta 6/secuencia 7 | Pruebas enfocadas canónicas de versión/instalador/instancia **10/10**; conjunto Release serial **451/451** y paralelo **451/451**; build Release **0 advertencias/0 errores**; `VE000 verified`, `publish` y smoke IPC integrado/explícito aprobados. El manifiesto y ZIP coincidieron en **495/495** rutas, hashes y contenido byte a byte; quedaron 0 entradas prohibidas, rutas fuente locales o CodeView y se conservaron las cinco capacidades. Evidencia automatizada local, no publicación ni prueba física |
| ZIP local del candidato beta 6 | `Trazio-Asistente-Reunion-v0.2.0-beta.6-win-x64.zip`: **86,830,246 bytes**, SHA-256 `c2c36402e63ecb45a5644f4d6fe0a9442fb5940c70c526984c740e4241e70b3a`. Es un artefacto local, no un recurso oficial |
| Setup local del candidato beta 6 | `Trazio-Asistente-Reunion-v0.2.0-beta.6-Setup.exe`: **60,002,700 bytes**, SHA-256 `01a674c4e0f6a12318e1ef678a5f64795039347e93024c15f79be6d97145b940`; Authenticode `NotSigned`; SHA-256 de manifiesto del payload `457317add99a2433ac3a34e6878e6f9ad53e9e88edca2790b5c61a84a2ec096c`. Compilado, pero nunca ejecutado con identidad productiva |
...
Evidencia local de beta 6/secuencia 7: las pruebas enfocadas canónicas de versión/instalador/instancia aprobaron **10/10**; los conjuntos Release serial y paralelo aprobaron **451/451** y el build Release terminó con **0 advertencias / 0 errores**. Windows PowerShell 5.1 publicó y validó el manifiesto de **495 archivos**: conjunto exacto, orden, rutas, longitudes y SHA-256. `VE000 verified`, `publish` y smoke IPC integrado/explícito aprobaron. El ZIP y layout coincidieron **495/495** archivos byte a byte. El Setup beta 6 local se compiló y sus sidecars se validaron, pero **el Setup con AppId productivo nunca se ejecutó**. El harness PS5.1 aprobó **14 escenarios**, devolvió el código exacto 5 de rollback, confirmó vivo el holder de `AppMutex` y limpió el estado desechable. La cancelación interactiva permanece `NOT_AUTOMATED`; ninguna de estas comprobaciones es una publicación ni validación física en otra máquina.
