...
La versión publicada actual es [`v0.2.0-beta.7`](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.7), una prerelease pública. El tag resuelve al commit `25e3f36867250599ef5026d7270fc37af85a7c44`. El ZIP oficial mide **86,851,547 bytes** y su SHA-256 es `951ce1653c2bbdd0d5c0a0827cab5b3c6d5c5c762f1574a3434fa937c44d00e8`; el Setup oficial mide **60,024,844 bytes** y su SHA-256 es `fb476b57e82fbf696e886831029be17d221596e5aaf0fd8cddf808714e0fe526`. Tamaños y digest de los recursos remotos coinciden con los artefactos verificados y sus archivos laterales. Se conserva la evidencia histórica de beta 3, beta 4, beta 5 y beta 6. Ninguna de estas comprobaciones sustituye evidencia física.

Beta 7 declara la secuencia de instalador **8** y está publicada. La coincidencia remota acredita los recursos observados, no la ejecución productiva del Setup, una instalación física ni las comprobaciones de hardware y duración pendientes.
...
La búsqueda local 8.2 está implementada en el candidato beta 8/secuencia 9 de `main`, no en la beta 7 publicada. Su conjunto enfocado Release aprobó **61/61** pruebas y la aplicación compiló con **0 advertencias / 0 errores**. Cubre texto efectivo corregido/deshecho, exclusión de revisiones alternativas, normalización, orden/límite, cancelación generacional, drenaje seguro al eliminar/cerrar, ausencia de esquema/índice nuevo, fallo cerrado ante corrupción y contrato de navegación sin reproducción automática. El candidato todavía no tiene tag, commit final, ZIP, Setup, tamaños, SHA-256 ni layout final verificado. Mantiene exactamente cinco capacidades; el instalador carece de firma Authenticode y el Setup productivo no se ha ejecutado. Tampoco sustituye pruebas físicas audibles, WGC/GPU, interfaz, teclado/lector de pantalla, Meet/Teams reales o 2/5 horas.
...
| Evidencia histórica de beta 6/secuencia 7 | Pruebas enfocadas **10/10**; Release serial/paralelo **451/451**; layout **495/495**. Fue la versión pública anterior y se conserva como antecedente verificable |
...
Como evidencia histórica, beta 6/secuencia 7 aprobó las pruebas enfocadas **10/10**, Release serial/paralelo **451/451** y un layout **495/495**. Fue la versión pública anterior y sus registros permanecen disponibles para trazabilidad.

Evidencia publicada de beta 7/secuencia 8: las pruebas enfocadas aprobaron **10/10**, Release serial y paralelo **520/520**, el build Release terminó con **0 advertencias / 0 errores** y `VE000 verified`. Windows PowerShell 5.1 aprobó `publish`, smoke IPC integrado/explícito y el harness desechable **14/14**. El ZIP coincide **495/495** con el payload y reporta `ProductVersion` `0.2.0-beta.7+25e3f36867250599ef5026d7270fc37af85a7c44` en App, Worker y `VisualAnalysis`; hubo 0 entradas prohibidas, rutas locales o CodeView local. El Setup permanece `NotSigned` y no fue ejecutado productivamente. El tag y los recursos remotos están publicados y verificados.
