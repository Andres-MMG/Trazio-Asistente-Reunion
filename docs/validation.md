...
| Candidato fuente del instalador manual offline — etapa 5.1 | Harness con identidades desechables: 14/14 escenarios aprobados — sidecars completos, aislamiento del build, manipulación rechazada, instalación limpia, reparación, A→B, rechazo B→A, secuencia cero, contradicción versión/secuencia, estado parcial, legacy desconocida, bloqueo por mutex, rollback y desinstalación. El rollback forzó un Abort real durante `[Files]`, obtuvo el código 5 de Inno Setup, mantuvo B activo y retiró los archivos parciales de C. Archivos arbitrarios fuera del árbol desechable permanecieron intactos; eso no demuestra supervivencia de la raíz real de datos. La cancelación humana permanece `NOT_AUTOMATED`. Este instalador todavía no es un recurso de la release pública beta 5 ni evidencia física en otro equipo |
...
Evidencia local final del candidato fuente: las pruebas enfocadas de versión/instalador/instancia aprobaron **10/10**; el conjunto Release aprobó **451/451** y la compilación Release terminó con **0 advertencias / 0 errores**. Windows PowerShell 5.1 publicó **495 archivos**, validó el conjunto exacto, orden, rutas, longitudes y SHA-256, y conservó `Trazio Asistente Reunión` como UTF-8 correcto. El build productivo volvió a publicar y compiló el Setup beta 5; todos los campos y formatos de sus sidecars coincidieron, pero **el Setup con AppId productivo no se ejecutó**. El harness PS5.1 aprobó **14 escenarios**, obtuvo el código exacto 5 de rollback, confirmó vivo el holder de `AppMutex` y limpió todo su estado desechable. La cancelación interactiva permanece `NOT_AUTOMATED`.
