...
La versión publicada actual es [`v0.2.0-beta.8`](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.8), una prerelease pública. El tag resuelve al commit `20c94272261f5697a548c56754039029b23f1548`. El ZIP oficial mide **86,866,781 bytes** y su SHA-256 es `240a792ab8388a0511fb8b25ac466feeb104fdf6779b72938302adc9570b2e3f`; el Setup oficial mide **60,034,031 bytes** y su SHA-256 es `61d87a71e2a040f10c70fa05389ca341db79e5cdcd88b2e314dcaf00c88fcaa0`. Tamaños y digest de los recursos remotos coinciden con los artefactos verificados y sus archivos laterales. Se conserva la evidencia histórica de beta 3 a beta 7. Ninguna de estas comprobaciones sustituye evidencia física.

Beta 8 declara la secuencia de instalador **9** y está publicada. La coincidencia remota acredita los recursos observados, no la ejecución productiva del Setup, una instalación física ni las comprobaciones de hardware y duración pendientes.

La beta 8 pública incluye la revisión de historial 8.1 y la búsqueda local 8.2. Sus pruebas automatizadas y artefactos publicados no sustituyen la validación audible, de interfaz, teclado/lector de pantalla, Meet/Teams reales ni las sesiones de 2/5 horas.

La búsqueda local 8.2 publicada cubre texto efectivo corregido/deshecho, exclusión de revisiones alternativas, normalización, orden/límite, cancelación generacional, drenaje seguro al eliminar/cerrar, ausencia de esquema/índice nuevo, fallo cerrado ante corrupción y navegación sin reproducción automática. El filtro enfocado actual de cinco clases aprobó **48/48**; el filtro ampliado **61/61** es evidencia histórica válida de esta función, no el conteo enfocado final. El conjunto completo Release aprobó **543/543** en serie y en paralelo, y los contratos finales aprobaron **22/22**. Mantiene exactamente cinco capacidades; el instalador carece de firma Authenticode y el Setup productivo no se ha ejecutado. Tampoco sustituye pruebas físicas audibles, WGC/GPU, interfaz, teclado/lector de pantalla, Meet/Teams reales o 2/5 horas.

La gestión global 8.3a está implementada en el candidato beta 9/secuencia 10 de `main` y todavía no forma parte de una release pública. El candidato no tiene tag, commit final, ZIP, Setup, tamaños, SHA-256 ni layout final definitivo. Sus pruebas enfocadas cubren orden/filtro/conteos, duplicados independientes, activación persistente sin recifrar blobs, Undo, cascada al eliminar, corrupción fail-closed, lifecycle cancelable, copy honesta y conservación de exactamente cinco capacidades. El instalador continúa `NotSigned`, el Setup productivo no se ha ejecutado y faltan validación visual, de teclado, lector de pantalla y pruebas físicas audibles, WGC/GPU, Meet/Teams y 2/5 horas.
...
Evidencia publicada de beta 8/secuencia 9: Release serial y paralelo **543/543**, filtro enfocado actual de cinco clases **48/48**, contratos finales **22/22** y harness desechable **14/14**. El filtro ampliado **61/61** se conserva como antecedente de 8.2. El ZIP coincide **495/495** con el payload y reporta `ProductVersion` `0.2.0-beta.8+20c94272261f5697a548c56754039029b23f1548` en App, Worker y `VisualAnalysis`; mantiene exactamente cinco capacidades. El Setup permanece `NotSigned` y no fue ejecutado productivamente. El tag y los recursos remotos están publicados y verificados.
...
| Identidad / selección de ventana / revisión / glosario / Markdown | [LocalProfileTests](../tests/Trazio.AsistenteReunion.Tests/LocalProfileTests.cs), [MeetingWindowSelectionTests](../tests/Trazio.AsistenteReunion.Tests/MeetingWindowSelectionTests.cs), [ReviewStoreTests](../tests/Trazio.AsistenteReunion.Tests/ReviewStoreTests.cs), [GlossaryCandidateExtractorTests](../tests/Trazio.AsistenteReunion.Tests/GlossaryCandidateExtractorTests.cs), [GlossaryWorkspacePresentationTests](../tests/Trazio.AsistenteReunion.Tests/GlossaryWorkspacePresentationTests.cs), [GlossaryWorkspacePublicationTests](../tests/Trazio.AsistenteReunion.Tests/GlossaryWorkspacePublicationTests.cs), [ObsidianMarkdownExportTests](../tests/Trazio.AsistenteReunion.Tests/ObsidianMarkdownExportTests.cs) |
...
- [ ] En el candidato beta 9, abrir **Diccionario**; comprobar orden reciente, ordinal, filtro de términos/categoría y estados Todos/Activos/Inactivos. Activar/desactivar, reiniciar y confirmar persistencia sin cambios de texto; probar duplicados por separado, fallo visible sin lista parcial y navegación/cierre durante una carga.
