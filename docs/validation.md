...
La versión publicada actual es [`v0.2.0-beta.4`](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.4), un prerelease no borrador. El tag corresponde al commit `f871f20c3bf9e77b0cf9ad51134febb83c673de7`. `0.2.0-beta.5` continúa como candidato local: sus pruebas, compilación, `publish` local, smoke IPC y layout preliminar fueron verificados de forma independiente, pero el ZIP debe reconstruirse desde el futuro commit de preparación. Por ello, tamaño/SHA-256 finales, SHA de preparación, tag y release pública beta 5 siguen pendientes. Ninguna de estas comprobaciones sustituye evidencia física.
...
| Pruebas y compilación del candidato beta 5 | `VersionMetadataTests` 4/4; `VisualEvaluation` 74/74; `Area=VisualCapture` 206/206; conjunto Release serial 446/446; conjunto Release paralelo 446/446; compilación con 0 advertencias y 0 errores; CLI `VE000 verified` |
| `Publish` local y smoke del candidato beta 5 | `Publish` local aprobado; smoke IPC integrado y smoke IPC explícito aprobados. Demuestran layout/arranque-respuesta del Worker, no captura WGC/GPU ni inferencia real |
| Layout preliminar del candidato beta 5 | 495/495 archivos coinciden byte a byte; `VisualAnalysis.dll` requerida y versionada con App/Worker; evaluator `.exe/.dll/.deps/.runtimeconfig`, corpus/golden y directorios `tools`/`evaluation` ausentes; exactamente cinco capacidades; 0 hallazgos prohibidos, 0 rutas fuente locales y 0 referencias CodeView |
| Activo final beta 5 | Pendiente: reconstruir después del commit de preparación y registrar tamaño/SHA-256 finales, SHA de preparación, tag y release. El layout preliminar no es el activo público final |
