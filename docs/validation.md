...
La versión publicada actual es [`v0.2.0-beta.10`](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.10), una prerelease pública, no un borrador. El tag resuelve al commit `52f8b016c277a5822e9aec269fc22cc055925a2e`. El ZIP oficial mide **86,897,741 bytes** y su SHA-256 es `52d5641af82e327bbfdf510dbd732d1dee7f13a7be5294af4b993fa1dc49a42f`; el Setup oficial mide **60,064,281 bytes** y su SHA-256 es `ce67f9ac2c05fa5718f99ef31339f74961af9de1a23a00c4a31ed56226977f2f`. Los seis recursos remotos y sus tamaños/digest coinciden con los artefactos verificados y sus archivos laterales. Se conserva la evidencia histórica de beta 3 a beta 9. Ninguna de estas comprobaciones sustituye evidencia física.
...
La gestión global 8.3a está publicada desde beta 9 y continúa en beta 10. Su evidencia de publicación original incluyó **23/23** pruebas enfocadas sobre orden/filtro/conteos, duplicados independientes, activación persistente sin recifrar blobs, Undo, cascada al eliminar, corrupción fail-closed, lifecycle cancelable, copy honesta y conservación de exactamente cinco capacidades. El conjunto Release de beta 9 aprobó **574/574** en serie y en paralelo; los contratos finales aprobaron **22/22** y el harness **14/14**. Activar una entrada todavía no modifica Whisper ni las transcripciones. El instalador continúa `NotSigned`, el Setup productivo no se ha ejecutado y faltan validación visual, de teclado, lector de pantalla y pruebas físicas audibles, WGC/GPU, Meet/Teams y 2/5 horas.
...
La rebanada 8.4a forma el candidato beta 11/secuencia 12 en la fuente actual de `main`, todavía no publicado. El filtro Release ampliado de nueve clases aprobó **77/77** pruebas, sin fallos ni omisiones; cubrió persistencia, cifrado, concurrencia, presentación, navegación, cancelación generacional, despacho de la consulta fuera del Dispatcher WPF, protección del borrador —incluidas ediciones creadas durante esperas asíncronas—, ciclo de vida, historial existente y conservación de exactamente cinco capacidades. El conjunto Release completo aprobó **628/628** tanto en serie como con paralelismo predeterminado, y la solución compiló en Release con **0 advertencias y 0 errores**; `git diff --check` terminó correctamente. Todavía no se creó tag, ZIP, Setup ni release para beta 11. La interacción física por teclado/lector de pantalla, el cierre interactivo y el error real de guardado siguen pendientes.
...
| Candidato fuente beta 11 con 8.4a, no publicado | Filtro Release ampliado de nueve clases **77/77**; conjunto Release serial/paralelo **628/628**; solución Release **0 advertencias/0 errores**; `git diff --check` correcto. No constituye evidencia física de teclado, lector de pantalla, audio, cierre interactivo o fallo real de guardado |
...
| Bandeja individual de revisión 8.4a | [BatchReviewStoreTests](../tests/Trazio.AsistenteReunion.Tests/BatchReviewStoreTests.cs), [BatchReviewPresentationTests](../tests/Trazio.AsistenteReunion.Tests/BatchReviewPresentationTests.cs), [BatchReviewPublicationTests](../tests/Trazio.AsistenteReunion.Tests/BatchReviewPublicationTests.cs) |
...
- [ ] Repetir la bandeja 8.4a por teclado y lector de pantalla: verificar foco, nombre/ayuda completos por fila y anuncio `Polite` de carga, vacío, truncamiento y error. Esta aceptación física sigue pendiente.
