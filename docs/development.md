...
Autoridad de versión de fuente: [Directory.Build.props](../Directory.Build.props). El candidato local actual declara `VersionPrefix` **0.2.0**, `VersionSuffix` **beta.6** (versión producto `0.2.0-beta.6`), `InstallerReleaseSequence` **7** y versión de ensamblado/archivo **0.2.0.0**. No tiene tag ni release: la versión pública vigente continúa siendo `0.2.0-beta.5`, secuencia **6**. Cada nueva versión instalable debe aumentar `InstallerReleaseSequence`; nunca compares SemVer beta como texto. El script de publicación, la definición del instalador y las [pruebas de versión/instalador](../tests/Trazio.AsistenteReunion.Tests/InstallerPackageContractTests.cs) comprueban el contrato.
...
### Evidencia local del candidato beta 6/secuencia 7

La validación local aprobó las pruebas enfocadas canónicas de versión/instalador/instancia **10/10**, Release serial **451/451**, Release paralelo **451/451**, compilación Release con **0 advertencias/0 errores**, `VE000 verified`, `publish` y smoke IPC integrado/explícito. Manifiesto/paquete coincidieron **495/495** archivos, rutas, hashes y contenido byte a byte; el harness desechable aprobó **14/14** escenarios, con rollback de código 5 y limpieza. El ZIP local (`Trazio-Asistente-Reunion-v0.2.0-beta.6-win-x64.zip`) mide **86,830,246 bytes**, SHA-256 `c2c36402e63ecb45a5644f4d6fe0a9442fb5940c70c526984c740e4241e70b3a`. El Setup local (`Trazio-Asistente-Reunion-v0.2.0-beta.6-Setup.exe`) mide **60,002,700 bytes**, SHA-256 `01a674c4e0f6a12318e1ef678a5f64795039347e93024c15f79be6d97145b940`, Authenticode `NotSigned`; el manifiesto de payload tiene SHA-256 `457317add99a2433ac3a34e6878e6f9ad53e9e88edca2790b5c61a84a2ec096c`. Son valores de artefactos locales, no de una release. Setup fue compilado, pero nunca ejecutado con identidad productiva; cancelación interactiva y validación en otra máquina/cuenta siguen pendientes.
...
La publicación histórica vigente sigue siendo `Trazio-Asistente-Reunion-v0.2.0-beta.5-win-x64.zip` junto a su archivo `.sha256`; su tamaño **86,829,207 bytes** y SHA-256 `0031cab096013b7bb436221a75d874719cc1d47ed03ac4047000b4e3094ddbdf` corresponden exclusivamente a ese recurso publicado. En una futura publicación, sustituye `<versión>` por la versión exacta del artefacto que quieras comprobar. Este comando solo calcula el hash de un archivo local: no crea un ZIP ni una versión publicada. El candidato beta 6 local y su Setup no son recursos publicados. Publicar, firmar y enviar cambios requieren autorización explícita del mantenedor.
