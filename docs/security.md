...
La fuente actual contiene el candidato local `v0.2.0-beta.6`, secuencia 7, aún sin tag ni release; la descarga pública continúa siendo beta 5. El Setup local se compiló sin firma (`NotSigned`) y no se ejecutó con identidad productiva. Sus sidecars y hashes son evidencia de integridad local, no autenticidad ni prueba de una instalación real.
