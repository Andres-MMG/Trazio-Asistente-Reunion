...
| Prompt de diccionario 8.5 | Solo la huella versionada del prompt confirmado se cifra con AES-256-GCM en la revisión; el prompt no se persiste | Términos descifrados y prompt existen transitoriamente en memoria administrada y durante el mensaje local al Worker |
| Notas, decisiones y seguimientos 8.6 | El contenido se cifra con AES-256-GCM y se elimina por cascada con la reunión | Tipo, estado, ID, relaciones e instantes permanecen como metadatos estructurales visibles; el texto descifrado existe en memoria e interfaz durante el uso |
| Decisiones de revisión 8.4a/8.4b | El nombre del revisor local se cifra con AES-256-GCM; la decisión solo referencia sesión/segmento y no copia título ni texto. El lote valida todo antes de insertar y revierte completo ante conflicto | Acción, revisión monotónica, ID, relación y fecha permanecen como metadatos estructurales visibles |
...
Fuentes: [Crypto.cs](../src/Trazio.AsistenteReunion.Core/Crypto.cs), [SettingsStore.cs](../src/Trazio.AsistenteReunion.Core/SettingsStore.cs), [almacén de sesiones](../src/Trazio.AsistenteReunion.Core/SqliteSessionStore.cs), [almacén de revisión](../src/Trazio.AsistenteReunion.Core/SqliteReviewStore.cs), [bandeja de revisión](../src/Trazio.AsistenteReunion.Core/SqliteBatchReviewStore.cs), [revisiones del modelo](../src/Trazio.AsistenteReunion.Core/SqliteModelRevisionStore.cs), [almacén de audio](../src/Trazio.AsistenteReunion.Core/AudioArchiveStore.cs).
...
- El diccionario global 8.3a descifra todas las entradas antes de publicar una lista y filtra términos/categorías en memoria con un máximo de 120 filas visibles. No crea FTS, índice, caché, registro ni preferencia con el filtro o sus resultados. Una entrada corrupta aborta la carga completa. Activar/desactivar modifica solo `is_active`; los blobs cifrados y su procedencia no cambian. En 8.5, las entradas activas se leen localmente al solicitar una retranscripción. Las formas ambiguas se excluyen y el usuario confirma, rechaza o cancela antes de enviar un prompt de hasta 1.024 caracteres al Worker local. Solo se persiste cifrada su huella versionada; no hay reemplazos automáticos. Los términos y el prompt existen transitoriamente en RAM administrada, que no ofrece borrado garantizado frente a volcados o software del mismo usuario.
- El intercambio 8.3b publicado en beta 10 importa como máximo 5 MiB/5.000 filas mediante una vista previa transitoria y revalida el diccionario dentro de la transacción antes de insertar todo o nada. Los términos importados se cifran con AES-GCM; solo se conserva un lote opaco, nunca nombre, ruta o hash del archivo. Los JSON exportados son texto plano deliberado: la interfaz advierte el riesgo y escribe primero un temporal hermano, pero el archivo final queda fuera de la protección y retención de Trazio. No se promete borrado seguro de la memoria administrada.
- La bandeja 8.4a/8.4b publicada en beta 12 conserva como máximo 100 segmentos visibles. Las casillas solo viven en memoria; el resumen de confirmación se muestra localmente. La aprobación múltiple cifra el revisor por decisión y usa una transacción todo-o-nada. No crea FTS, índices, caché persistente, tráfico de red ni registros con texto.
- Las anotaciones 8.6 cifran el contenido y conservan en texto estructural solo tipo, estado, relaciones e instantes. La creación es manual, completar/reabrir usa control de estado esperado y eliminar exige confirmación. Una etiqueta AES-GCM inválida aborta la carga completa de anotaciones de la sesión. No se envían por red ni se incorporan automáticamente a TXT, Markdown, Obsidian o inteligencia externa.
...
La versión pública actual es [v0.2.0-beta.13](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.13). El payload mantiene exactamente cinco capacidades y el Setup continúa NotSigned. Esta evidencia no cambia Unvalidated, **No disponible** ni la ausencia de identificación de hablantes.

El ZIP publicado mide **86,897,741 bytes** y su SHA-256 es `52d5641af82e327bbfdf510dbd732d1dee7f13a7be5294af4b993fa1dc49a42f`. El Setup publicado mide **60,064,281 bytes**, su SHA-256 es `ce67f9ac2c05fa5718f99ef31339f74961af9de1a23a00c4a31ed56226977f2f` y Authenticode informa `NotSigned`. El manifiesto de publicación mide **123,577 bytes** y su SHA-256 es `f5fb80b5dce7b32dd478f2b062de3adf40d96a2db0dc254d8b6c4e1c6d6d81f4`; el manifiesto del Setup mide **488 bytes** y su SHA-256 es `f867fc695aa9de37e266621de825958f8ef9967be989cdfd2829c83d4a197a01`. Los seis recursos remotos y sus tamaños/digest coinciden con los artefactos verificados. Esto prueba integridad respecto de esos valores, no autenticidad del editor ni una instalación real: el Setup con identidad productiva no se ejecutó, la cancelación humana continúa `NOT_AUTOMATED` y sigue pendiente la validación en otra máquina o cuenta.
...
- Las exportaciones heredadas de reuniones pueden dejar un archivo incompleto **sin cifrar** si fallan después de abrir el destino. La exportación JSON del diccionario en 8.3b escribe un temporal hermano y solo reemplaza al completar; si se cancela o falla conserva el destino previo y limpia ese temporal. Todo archivo final exportado sigue siendo sensible y queda fuera del cifrado de Trazio.
