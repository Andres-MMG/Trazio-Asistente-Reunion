...
| Decisiones de revisión 8.4a | El nombre del revisor local se cifra con AES-256-GCM; la decisión solo referencia sesión/segmento y no copia título ni texto | Acción, revisión monotónica, ID, relación y fecha permanecen como metadatos estructurales visibles |
...
- El diccionario global 8.3a descifra todas las entradas antes de publicar una lista y filtra términos/categorías en memoria con un máximo de 120 filas visibles. No crea FTS, índice, caché, registro ni preferencia con el filtro o sus resultados. Una entrada corrupta aborta la carga completa. Activar/desactivar modifica solo el metadato `is_active`; los blobs cifrados y su procedencia no cambian. Una entrada activa queda preparada para una aplicación futura: beta 10 todavía no modifica Whisper ni las transcripciones. Esos textos descifrados existen transitoriamente en RAM con los mismos límites frente a paginación, volcados o software del usuario.
...
- La bandeja 8.4a incluida en el candidato beta 11 de `main`, todavía no publicado, conserva como máximo 100 segmentos visibles, pero termina de leer y autenticar cada candidato para calcular truncamiento y rechazar toda la carga si encuentra contenido cifrado corrupto. No crea FTS, índice de texto, caché persistente, archivo de resultados, tráfico de red ni registro con texto o identificadores. Títulos, transcripciones y nombre del revisor se descifran transitoriamente en RAM administrada; no se promete borrado seguro ni protección frente a paginación, volcados o software con el mismo acceso de usuario.
...
La versión pública actual es [`v0.2.0-beta.10`](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.10). Su layout final **495/495**, con `ProductVersion` `0.2.0-beta.10+52f8b016c277a5822e9aec269fc22cc055925a2e`, incluye `VisualAnalysis.dll` requerida/versionada y excluye evaluador, corpus/golden y directorios `tools`/`evaluation`; conserva exactamente cinco capacidades y cero hallazgos prohibidos, rutas locales o CodeView. El tag resuelve a `52f8b016c277a5822e9aec269fc22cc055925a2e`. Esta evidencia no cambia `Unvalidated`, **No disponible** ni la ausencia de identificación de hablantes.
