...
- Versión publicada actual: [`0.2.0-beta.10`](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.10), secuencia de instalador **11**
- Candidato en `main`: `0.2.0-beta.11`, secuencia de instalador **12**; todavía no publicado
- Versión anterior: `0.2.0-beta.9`, secuencia de instalador **10**; se conserva como evidencia histórica
- Antecedente adicional: `0.2.0-beta.8`, secuencia de instalador **9**

Este es el plan canónico de etapas. **La beta 10/secuencia 11 publica el instalador manual offline por usuario de la etapa 5.1, la revisión de historial 8.1, la búsqueda local 8.2, el empaquetador ZIP canónico, la gestión global básica del diccionario 8.3a y el intercambio JSON 8.3b.** La primera rebanada individual 8.4a de pendientes de revisión forma el candidato beta 11/secuencia 12 en `main`; todavía no forma parte de la descarga pública. El ZIP y el Setup de beta 10 están verificados; el Setup permanece sin firma y no se ejecutó con identidad productiva. El paquete conserva exactamente cinco capacidades. Siguen pendientes cancelación humana, validación física audible, importación/exportación real, teclado/lector de pantalla, WGC/GPU, Meet/Teams reales, 2/5 horas, otra máquina o cuenta y autoactualización. La etapa 6 tiene una línea base funcional implementada; 7.1a, 7.2a y la infraestructura de 7.2b están publicadas, todavía pendientes de validación física. Beta 10 distribuye el ensamblado compartido puro y sin paquetes `VisualAnalysis`; el evaluador offline, el corpus sintético agregado y el golden canónico permanecen en el código fuente y fuera del paquete. Esa regresión sintética no valida superficies reales. 7.2b falla de forma segura en producción porque los perfiles Meet/Teams permanecen `Unvalidated`: se abstiene y presenta la evidencia como No disponible. No existe identificación de hablantes, no se agrega una capacidad empaquetada y la etapa 7 no está completa. La identidad local (5.5) está implementada, todavía sin validación física. Activar una entrada del diccionario todavía no modifica Whisper ni las transcripciones. Beta 9, beta 8, beta 7 y beta 6 quedan como antecedentes históricos. Consulta la [documentación de ingeniería](docs/README.md), el [corpus sintético](evaluation/stage-7b/README.md) y la [evidencia de validación](docs/validation.md). El alcance futuro indicado a continuación es un objetivo, no una afirmación de que ya se distribuya.
...
| 5 | Beta distribuible y mantenible | Etapa 5.1 vigente en beta 10/secuencia 11 — firma, ejecución productiva del Setup, validación física y cancelación humana pendientes |
...
| 7 | Fuente de reunión y atribución de hablantes | 7.1a/7.2a y la infraestructura de 7.2b publicadas desde beta 6 y vigentes en beta 10; evaluador/corpus sintético presentes solo en fuente — perfiles de producción no validados, sin identificación de hablantes y con validación física pendiente; 7.2c+ planificadas |
| 8 | Búsqueda, revisión por lotes, glosario global y productividad | En curso — 8.1, 8.2, 8.3a y 8.3b incluidas en beta 10; 8.4a individual incluida en el candidato beta 11 de `main`; selección múltiple, aprobación masiva, aplicación del diccionario y validación física siguen pendientes |
...
- Código público en GitHub, ZIP completo y Setup manual para Windows con sumas de comprobación; [`0.2.0-beta.10`](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.10) está publicado como prerelease. El tag resuelve al commit `52f8b016c277a5822e9aec269fc22cc055925a2e`. El ZIP de **86,897,741 bytes**, SHA-256 `52d5641af82e327bbfdf510dbd732d1dee7f13a7be5294af4b993fa1dc49a42f`, y el Setup de **60,064,281 bytes**, SHA-256 `ce67f9ac2c05fa5718f99ef31339f74961af9de1a23a00c4a31ed56226977f2f`, coinciden con los recursos remotos y sus archivos laterales.
...
8.4a añade decisiones anexadas `ApproveOriginal`/`Reopen` con revisión monotónica por segmento y revisor local cifrado. La escritura revalida sesión, segmento, estado elegible, ausencia de correcciones y revisión esperada dentro de una transacción; no reintenta conflictos silenciosamente. La consulta no usa FTS, índices de texto ni caché persistente, y una corrupción autenticada invalida la carga completa.
