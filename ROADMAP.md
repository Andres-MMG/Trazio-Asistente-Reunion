...
- Versión publicada actual: [0.2.0-beta.13](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.13), secuencia de instalador **14**
- Versión anterior: `0.2.0-beta.12`, secuencia de instalador **13**; se conserva como evidencia histórica
...
Este es el plan canónico de etapas. **La beta 13/secuencia 14 publica la aplicación controlada 8.5 del diccionario sobre retranscripciones versionadas.** El ZIP y el Setup están verificados; el Setup permanece sin firma y no se ejecutó con identidad productiva. El paquete conserva exactamente cinco capacidades. Siguen pendientes cancelación humana, validación física audible y de accesibilidad, WGC/GPU, Meet/Teams reales, 2/5 horas, otra máquina o cuenta y autoactualización. La etapa 6 tiene una línea base funcional; 7.1a, 7.2a y la infraestructura de 7.2b están publicadas, todavía pendientes de validación física. No existe identificación de hablantes. Una entrada activa solo orienta Whisper cuando el usuario confirma explícitamente una retranscripción; no reemplaza texto ni altera el original.
...
| 5 | Beta distribuible y mantenible | Etapa 5.1 vigente en beta 13/secuencia 14 — firma, ejecución productiva del Setup, validación física y cancelación humana pendientes |
...
| 8 | Búsqueda, revisión por lotes, glosario global y productividad | En curso — 8.1 a 8.4b permanecen vigentes y 8.5 se publica en beta 13 con aplicación confirmada del diccionario a retranscripciones; validación física y revisión avanzada de hablantes siguen pendientes |
...
- Código público en GitHub, ZIP completo y Setup manual para Windows con sumas de comprobación; [`0.2.0-beta.13`](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.13) está publicado como prerelease. El tag resuelve al commit `512d6cccc631f22505d52f62cfbb0443db42394a`. El ZIP de **86,940,830 bytes**, SHA-256 `a5d8d83d048167e2d9ccfa1ad21ed676278af49bd00253c206bf7bdda8ec5220`, y el Setup de **60,084,574 bytes**, SHA-256 `405c4292294933628736b060009bd4a321139fec31adc6622540530f8c2d4eec`, coinciden con los recursos remotos y sus archivos laterales.
...
| Glosario con procedencia | Extrae términos modificados después de una edición humana y guarda de forma explícita entradas cifradas enlazadas a su corrección de origen. En 8.5, una entrada activa puede orientar una retranscripción después de confirmación explícita; no cambia grabaciones nuevas ni reemplaza texto. |
...
- La navegación anterior/siguiente, la velocidad y el resaltado continuo se publicaron como 8.1 en beta 7; la revisión individual y múltiple 8.4a/8.4b están publicadas, con validación física pendiente.
...
8.3a reutiliza el esquema cifrado existente y solo actualiza `is_active`; no recifra ni modifica término, categoría, procedencia o fecha. Filtro, consulta y resultados son transitorios en memoria, sin FTS, caché, configuración ni capacidad nueva. **Activa** significa elegible para la confirmación explícita de 8.5; no reemplaza texto ni cambia una transcripción por sí sola. Borrar una sesión también elimina por cascada las entradas originadas en sus correcciones.
