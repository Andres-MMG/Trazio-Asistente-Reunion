...
- Versión publicada actual: [`0.2.0-beta.8`](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.8), secuencia de instalador **9**
- Candidato actual en `main`: `0.2.0-beta.9`, secuencia de instalador **10**; todavía sin tag, artefactos ni publicación
- Versión anterior: `0.2.0-beta.7`, secuencia de instalador **8**; se conserva como evidencia histórica

Este es el plan canónico de etapas. **La beta 8/secuencia 9 publica el instalador manual offline por usuario de la etapa 5.1, la revisión de historial 8.1, la búsqueda local 8.2 y el empaquetador ZIP canónico. El candidato beta 9/secuencia 10 de `main` incorpora 8.3a, la gestión global básica del diccionario, y todavía no está publicado.** El ZIP y el Setup de beta 8 están verificados; el Setup permanece sin firma y no se ejecutó con identidad productiva. El candidato beta 9 aún no tiene tag, commit final, ZIP, Setup, tamaños ni hashes definitivos y conserva exactamente cinco capacidades. Siguen pendientes cancelación humana, validación física audible, teclado/lector de pantalla, WGC/GPU, Meet/Teams reales, 2/5 horas, otra máquina o cuenta y autoactualización. La etapa 6 tiene una línea base funcional implementada; 7.1a, 7.2a y la infraestructura de 7.2b están publicadas, todavía pendientes de validación física. Beta 8 distribuye el ensamblado compartido puro y sin paquetes `VisualAnalysis`; el evaluador offline, el corpus sintético agregado y el golden canónico permanecen en el código fuente y fuera del paquete. Esa regresión sintética no valida superficies reales. 7.2b falla de forma segura en producción porque los perfiles Meet/Teams permanecen `Unvalidated`: se abstiene y presenta la evidencia como No disponible. No existe identificación de hablantes, no se agrega una capacidad empaquetada y la etapa 7 no está completa. La identidad local (5.5) está implementada, todavía sin validación física. Beta 7 y beta 6 quedan como antecedentes históricos. Consulta la [documentación de ingeniería](docs/README.md), el [corpus sintético](evaluation/stage-7b/README.md) y la [evidencia de validación](docs/validation.md). El alcance futuro indicado a continuación es un objetivo, no una afirmación de que ya se distribuya.
...
| 5 | Beta distribuible y mantenible | Etapa 5.1 publicada en beta 8/secuencia 9 — firma, ejecución productiva del Setup, validación física y cancelación humana pendientes |
...
| 7 | Fuente de reunión y atribución de hablantes | 7.1a/7.2a y la infraestructura de 7.2b publicadas desde beta 6 y vigentes en beta 8; evaluador/corpus sintético presentes solo en fuente — perfiles de producción no validados, sin identificación de hablantes y con validación física pendiente; 7.2c+ planificadas |
| 8 | Búsqueda, revisión por lotes, glosario global y productividad | En curso — 8.1 y 8.2 incluidas en beta 8; 8.3a incluida en el candidato beta 9 de `main`, todavía no publicado; validación física, revisión por lotes y gestión avanzada siguen pendientes |
...
- Código público en GitHub, ZIP completo y Setup manual para Windows con sumas de comprobación; [`0.2.0-beta.8`](https://github.com/Andres-MMG/Trazio-Asistente-Reunion/releases/tag/v0.2.0-beta.8) está publicado como prerelease. El tag resuelve al commit `20c94272261f5697a548c56754039029b23f1548`. El ZIP de **86,866,781 bytes**, SHA-256 `240a792ab8388a0511fb8b25ac466feeb104fdf6779b72938302adc9570b2e3f`, y el Setup de **60,034,031 bytes**, SHA-256 `61d87a71e2a040f10c70fa05389ca341db79e5cdcd88b2e314dcaf00c88fcaa0`, coinciden con los recursos remotos y sus archivos laterales.
...
- Beta 8 incluye un instalador Inno Setup manual/offline por usuario. Mantiene una raíz estable y payloads en `versions/<versión>`, repara la misma secuencia, actualiza solo hacia secuencias mayores y rechaza downgrades o versiones legacy desconocidas antes de copiar.
...
- Beta 8/secuencia 9: Release serial/paralelo **543/543**; filtro enfocado actual de cinco clases **48/48**; contratos finales **22/22**; harness desechable **14/14**. El filtro ampliado **61/61** se conserva como evidencia histórica de 8.2. El layout final coincide en **495/495** archivos y reporta `ProductVersion` `0.2.0-beta.8+20c94272261f5697a548c56754039029b23f1548`; mantiene exactamente cinco capacidades. El Setup publicado informa `NotSigned` y no se ejecutó productivamente; tampoco se completaron cancelación humana ni validación en otra máquina o cuenta.
...
- **8.3b pendiente:** detección y fusión asistida de duplicados, importación y exportación con vista previa.
